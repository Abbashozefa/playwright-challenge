...
async def scroll_and_load(page):
    ...
    while True:
        ...
        updated_count = await page.locator("div.text-sm.text-muted-foreground span.font-medium.text-foreground:nth-of-type(1)").text_content()
        # if displayed_count == current_count:pr
        #     continue
        if int(displayed_count) <= int(updated_count) and int(updated_count) < int(total_count) :
            print(displayed_count,total_count)
